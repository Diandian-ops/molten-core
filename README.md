...
resources/
  ...
  enemies/                # 敌人数值配置 (.tres)：灼奴 / 熔壳兵 / 裂痕使者 / 余烬飞妖
levels/                  # 关卡数据 (.tres)：level_01 ~ level_05，波次/裂痕点/星级阈值
...
- 5 个示例关卡（序章 / 第一章 / 第二章 / 第三章·熔火之心 / 终章·熔心王座），每关波次末尾接入一个 Boss。
...
- Boss 多阶段：4 个 Boss（熔岩巨像 / 暗影行者 / 深渊领者 / 熔心君主），随血线触发加速、召唤小怪等阶段技能。
...
- **剧情日志系统（新增）**：左列表（按关卡解锁进度渐进解锁）+ 右详情；文案来自 `resources/story/` 下 8 篇 `StoryLog` 资源（序章 / 世界观·起源 / 守炉人 / 第一~三章 / 尾声 / 终章），数据驱动、可在编辑器内增改。
- **结算面板风格统一**：动态余烬背景 + 面板缩放入场 + 星标弹跳；胜利时提示「通关解锁新剧情」。

> 说明：v0.5.0 的「终局关卡 level_05（终章·熔心王座）+ 终局 Boss 熔心君主」已实装，剧情日志扩展至 8 篇。BGM 钩子已接入、缺音乐资产。导出构建（`build/`）仍为 v0.4.0，待 push 后由 CI 重新导出时一并更新。

## 后续方向（尚未实现）

- 移动端适配（触屏 UI + 虚拟按钮，iOS / Android 导出）。
- 背景音乐（BGM）音乐文件资产（钩子已接入，放入 `assets/audio/bgm/` 即启用）。
- 更多关卡：第 5 关（终章·熔心王座 + 终局 Boss 熔心君主）已实装。

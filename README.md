...
levels/                  # 关卡数据 (.tres)：level_01 ~ level_04，波次/裂痕点/星级阈值
...
- 4 个示例关卡（序章 / 第一章 / 第二章 / 终章·熔火之心），每关波次末尾接入一个 Boss。
...
> 说明：v0.4.0 的「新关卡 level_04（终章·熔火之心）」已实装；第 5 关待做。BGM 钩子已接入、缺音乐资产。其余 UI / 世界观内容已完成。导出构建（`build/`）仍为 v0.3.0，下次重新导出时一并更新。

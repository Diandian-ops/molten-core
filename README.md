...
- 塔防核心循环：裂痕点按波次生成敌人 → 敌人移动向熔核 → 守卫塔攻击/减速 → 击杀掉落晶币 → 熔核能量归零判负 / 打完所有波次判胜。
- 数据驱动：关卡、敌人、守卫塔均为 `.tres` 资源，策划可视化调整数值。
- 存档：关卡星级与解锁进度保存在 `user://molten_core_save.json`。
- 3 个示例关卡（对应 GDD 中序章、第一章、第二章的雏形节奏）。

## 尚未实现（占位美术/后续方向）

- ~~当前所有视觉表现均为几何占位图形（`Polygon2D`/`ColorRect`），未接入正式美术资源。~~ **已集成 [Kenney Tower Defense Pack](https://kenney.nl/assets/tower-defense-top-down) 素材**：守卫塔、敌人（UFO 飞碟）、熔核（水晶）、建造点均已替换为 64x64 像素艺术精灵图。
- 塔的升级树、熔核主动技能、剧情碎片收集、音效音乐等在 GDD 的「后续可扩展方向」中列出，尚未实现。
...
两个平台均已使用 Godot 4.7.1 导出模板构建，可直接运行。
